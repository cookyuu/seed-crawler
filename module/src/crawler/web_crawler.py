...
class WebCrawler:
    ...
    def crawl(self, event: JobCrawlRequestEvent) -> JobCrawlResultEvent:
        ...
        while retry_count <= event.retry_limit:
            if retry_count > 0:
                logger.info(
                    f"Retrying ({retry_count}/{event.retry_limit}) for jobId: {event.job_id}"
                )
                time.sleep(event.retry_interval_sec)

            try:
                result = self._execute_request(event)
                ...
                last_error = str(e)
                logger.warning(
                    f"Request failed for jobId: {event.job_id}, attempt: {retry_count + 1}, error: {e}"
                )
    ...
    def _execute_request(self, event: JobCrawlRequestEvent) -> requests.Response:
        ...
        method = event.job_execution_type.value
